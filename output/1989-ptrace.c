/Users/wpf/Desktop/log/test/linux-2.6.9/arch/ia64/kernel/ptrace.c
static long
{
get_user(pt->cr_iip, &ppr->cr_iip);
get_user(psr, &ppr->cr_ipsr);
get_user(pt->ar_pfs, &ppr->ar[PT_AUR_PFS]);
get_user(pt->ar_rsc, &ppr->ar[PT_AUR_RSC]);
...
copy_from_user(&pt->f6, &ppr->fr[6], sizeof(ppr->fr[6]) * 6);
copy_from_user(&sw->f12, &ppr->fr[12], sizeof(ppr->fr[12]) * 4);
copy_from_user(&fpval, &ppr->fr[i], sizeof(fpval));
copy_from_user(&child->thread.fph, &ppr->fr[32], sizeof(ppr->fr[32]) * 96);
