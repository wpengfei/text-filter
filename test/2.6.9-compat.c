			       ...
			       unsigned char *stackbuf, int stackbuf_size)
{
	...
	while(ucmsg != NULL) {  //第一个循环做参数检查
		...
		if((unsigned long)(((char __user *)ucmsg - (char __user *)kmsg->msg_control)
			...
			return -EINVAL;                       //此处对参数长度进行了校验，但在循环2中没有
	...
	ucmsg = CMSG_COMPAT_FIRSTHDR(kmsg);
	while(ucmsg != NULL) { //第二个循环将数据拷贝进内核结构
		__get_user(ucmlen, &ucmsg->cmsg_len);         //[6] //即代码在[2]~[6]执行时，ucmlen值有被更改的风险
		tmp = ((ucmlen - CMSG_COMPAT_ALIGN(sizeof(*ucmsg))) +
		       CMSG_ALIGN(sizeof(struct cmsghdr)));
		kcmsg->cmsg_len = tmp;
		...
		/* Copy over the data. */
		if(copy_from_user(CMSG_DATA(kcmsg),    //[7]  在[7]处的数据复制会导致overflow
